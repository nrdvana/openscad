			...
			if (matrix_contains_infinity(node.matrix) || matrix_contains_nan(node.matrix)) {
				...
				if ((geom = res.constptr())) {
					if (geom->getDimension() == 2) {
						...
						newpoly->matrix = node.matrix * newpoly->matrix;
						geom = newpoly;
					}
					else {
...
shared_ptr<const Geometry> GeometryEvaluator::extrudePolygonSequence(const ExtrudeForNode &node, const std::vector<const class Polygon2dIn3d *> &slices)
{
	...
	if (slices.size() < 2) {
		PRINTB("ERROR: %s requires at least two iterations, %s", node.name() % loc);
		return nullptr;
	}
	...
	for (auto slice : slices) {
		...
		if (!match) {
			PRINTB("ERROR: Each extrusion_shape must have exactly the same number of polygons of the same vertex count, %s", loc);
			return nullptr;
		}
	}

	// Start extruding slices.  Come back to "end caps" at the end.
	PolySet tmp0(3), tmp1(3), tmp2(3), *result = new PolySet(3);
	// Unroll first iteration so we have a "prev" to work with, and so we can use it again at the end
	for (const auto &outline : slice0->outlines()) {
		tmp0.append_poly();
		for (const auto &vtx : outline.vertices)
			tmp0.append_vertex(vtx[0], vtx[1], 0);
	}
	tmp0.transform(slice0->matrix);
	
	PolySet *cur= &tmp1, *prev= &tmp0;
	for (i= 1; i < slices.size(); i++, prev= cur, cur= (cur == &tmp1? &tmp2 : &tmp1)) {
		// Build new polygon set in 3D from 2D outlines.  This doesn't get added to the 
		cur->polygons.clear();
		for (const auto &outline : slices[i]->outlines()) {
			cur->append_poly();
			for (const auto &vtx : outline.vertices)
				cur->append_vertex(vtx[0], vtx[1], 0);
		}
		cur->transform(slices[i]->matrix);
		...
		if (i == slices.size()-1) {
			...
			if (closed_loop) // use exact original coordinates
				...
				PolySet *start= slice0->tessellate();
				for(auto &p : start->polygons) std::reverse(p.begin(), p.end());
				start->transform(slice0->matrix);
				result->append(*start);
				delete start;

				PolySet *end= slices[i]->tessellate();
				end->transform(slices[i]->matrix);
				result->append(*end);
				delete end;
			}
		}
		...
		Vector3d cur_origin(slices[i]->matrix * Vector3d(0,0,0));
		Vector3d cur_abc(slices[i]->matrix * Vector3d(0,0,-1) - cur_origin);
		double cur_d= - (cur_abc.dot(cur_origin));
		Vector3d prev_origin(slices[i-1]->matrix * Vector3d(0,0,0));
		Vector3d prev_abc(slices[i-1]->matrix * Vector3d(0,0,1) - prev_origin);
